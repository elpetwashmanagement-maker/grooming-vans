export default async function handler(req, res) {
  ...
  try {
    ...
    if (client?.id) {
      ...
      console.log('Appts found:', JSON.stringify(appts), 'for date:', tomorrowISO);
    ...
    console.log('Company resolved:', companyId, 'fromNumber:', fromNumber);
    ...
    await fetch('https://lpzwnbrjpayjhlwjmuda.supabase.co/rest/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        phone: fromPhone, body, direction: 'inbound',
        company_id: companyId, client_id: client?.id || null,
        client_name: client?.name || null, message_id: msg.id,
