...
export default function App() {
  ...
  if (!session) {
    return <LoginScreen vans={vans} adminPin={settings.adminPin} onLogin={setSession} />;
  }
  ...
  return (
    <div style={styles.app}>
      ...
      <footer style={styles.footer}>
        <Sparkles size={12} /> Cierre Diario · Grooming Vans
      </footer>
    </div>
  );
}
...
function LoginScreen({ vans, adminPin, onLogin }) {
  ...
  const tryLogin = (pin) => {
    if (isAdminMode) {
      ...
      if (pin === selectedVan.pin) {
        onLogin({ type: 'van', vanId: selectedVan.id });
      } else {
        setError(true);
        setTimeout(() => { setPinInput(''); setError(false); }, 600);
      }
    ...
    <div style={styles.loginScreen}>
      ...
      <div style={styles.loginCard}>
        <div style={styles.loginHeader}>
          ...
          <div>
            <h1 style={styles.title}>Grooming Vans</h1>
            <p style={styles.subtitle}>
              {step === 'select' ? 'Selecciona tu van' : isAdminMode ? 'Acceso administrador' : `Hola ${selectedVan?.groomer || selectedVan?.name}`}
            </p>
          </div>
        </div>
        ...
        {step === 'select' ? (
          <div style={{ animation: 'fadeIn 0.3s ease' }}>
            <div style={styles.vanTilesGrid}>
              {vans.map(v => (
                <button
                  key={v.id}
                  onClick={() => handleSelectVan(v)}
                  className="van-tile"
                  style={styles.vanTile}
                >
                  <div style={styles.vanTileIcon}>
            ...
            <button onClick={handleSelectAdmin} style={styles.adminAccessBtn}>
              <Lock size={15} />
          ...
          <div style={{ animation: 'fadeIn 0.3s ease' }}>
            ...
            {error && (
              <p style={styles.pinError}>PIN incorrecto, intenta de nuevo</p>
            )}

            <div style={styles.pinPad}>
              {['1','2','3','4','5','6','7','8','9'].map(d => (
                <button key={d} onClick={() => handleDigit(d)} className="pin-btn" style={styles.pinBtn}>
                  {d}
                </button>
              ))}
              <button onClick={() => setShowPin(!showPin)} className="pin-btn" style={{ ...styles.pinBtn, fontSize: 14 }}>
                {showPin ? <EyeOff size={20} /> : <Eye size={20} />}
              </button>
              <button onClick={() => handleDigit('0')} className="pin-btn" style={styles.pinBtn}>0</button>
              <button onClick={handleDelete} className="pin-btn" style={{ ...styles.pinBtn, fontSize: 14 }}>
                ⌫
              </button>
            </div>

            <button onClick={goBack} style={styles.pinBackBtn}>
              ← Volver
            </button>
          </div>
        )}
...
function Header({ tab, setTab, isAdmin, currentVan, onLogout }) {
  const allTabs = [
    { id: 'registro', label: 'Registro', icon: Plus, vanAccess: true, adminAccess: true },
    { id: 'cierre', label: 'Mi Cierre Diario', icon: FileText, vanAccess: true, adminAccess: false },
    { id: 'cierre', label: 'Cierre Diario', icon: FileText, vanAccess: false, adminAccess: true },
    { id: 'semana', label: 'Reporte Semanal', icon: TrendingUp, vanAccess: false, adminAccess: true },
    { id: 'config', label: 'Configuración', icon: SettingsIcon, vanAccess: false, adminAccess: true },
  ];
  const tabs = allTabs.filter(t => isAdmin ? t.adminAccess : t.vanAccess);
          ...
          <div>
            ...
            <p style={styles.subtitle}>
              {isAdmin
                ...
                : `${currentVan?.name}${currentVan?.groomer ? ' · ' + currentVan.groomer : ''}`
              }
            </p>
          </div>
        </div>
...
function RegistroTab({ vans, services, setServices, fixedVanId }) {
  ...
  const [form, setForm] = useState({
    client: '', pet: '', service: '', method: 'Efectivo', amount: '', tip: '',
  });
  const [editingId, setEditingId] = useState(null);
  const [showSuggestions, setShowSuggestions] = useState(false);

  // Si la van está fija (no admin), forzar siempre ese vanId
  useEffect(() => {
    if (fixedVanId) setVanId(fixedVanId);
  }, [fixedVanId]);

  // Construye lista única de clientes recurrentes con su última info
    ...
    [...services]
      ...
      .forEach(s => {
        ...
        map.set(key, {
          ...
          pet: s.pet?.trim() || prev.pet || '',
          lastService: s.service?.trim() || prev.lastService || '',
          lastAmount: s.amount || prev.lastAmount || 0,
          count: prev.count + 1,
        });
      });
  ...
  const selectClient = (c) => {
    setForm(f => ({
      ...
      client: c.client,
      pet: f.pet || c.pet,
      service: f.service || c.lastService,
      amount: f.amount || (c.lastAmount ? String(c.lastAmount) : ''),
    }));
  ...
  const dayServices = useMemo(() =>
    services
      .filter(s => s.date === date && s.vanId === vanId)
  ...
  const handleSubmit = () => {
    if (!form.client.trim() || !form.amount) {
      alert('Completa al menos cliente y monto');
      return;
    }
    if (editingId) {
  ...
  const handleEdit = (s) => {
    setForm({
      ...
      method: s.method, amount: String(s.amount), tip: String(s.tip || ''),
    });
    setEditingId(s.id);
  };
  ...
  return (
    <div style={{ animation: 'fadeIn 0.3s ease' }}>
      <SectionTitle eyebrow="Paso 1" title="Registrar servicio" />
      ...
      <div style={styles.card}>
        ...
        <div style={styles.formGrid}>
            ...
            <label style={styles.lbl}>
              ...
              {knownClients.length > 0 && (
                <span style={{ marginLeft: 8, fontSize: 10, color: '#0f766e', fontWeight: 700, textTransform: 'none', letterSpacing: 0 }}>
                  · {knownClients.length} cliente{knownClients.length === 1 ? '' : 's'} guardado{knownClients.length === 1 ? '' : 's'}
                </span>
              )}
            </label>
            <input
              value={form.client}
              onChange={e => { setForm({ ...form, client: e.target.value }); setShowSuggestions(true); }}
            ...
            {showSuggestions && clientSuggestions.length > 0 && (
              <div style={styles.suggestionsBox}>
                <div style={styles.suggestionsHeader}>
                  Clientes encontrados · toca para autocompletar
                </div>
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
                      ...
                      <div style={{ textAlign: 'right', flexShrink: 0 }}>
                        {c.lastAmount > 0 && (
                          <div style={{ fontWeight: 600, color: '#0f766e', fontSize: 13 }}>{fmt(c.lastAmount)}</div>
                        )}
                        <div style={{ fontSize: 10, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.06em', marginTop: 2 }}>
          ...
          <div>
            ...
            <input value={form.service} onChange={e => setForm({ ...form, service: e.target.value })} style={styles.input} placeholder="Ej: Baño completo" />
          </div>
          <div>
                  ...
                  {dayServices.map(s => (
                    <tr key={s.id} className="row-hover" style={styles.tr}>
                      ...
                      <td style={{ ...styles.td, textAlign: 'right' }}>
                        <button onClick={() => handleEdit(s)} style={styles.iconBtn} title="Editar"><Edit2 size={14} /></button>
                        <button onClick={() => handleDelete(s.id)} style={{ ...styles.iconBtn, color: '#dc2626' }} title="Eliminar"><Trash2 size={14} /></button>
                      </td>
                    </tr>
                  ))}
                </tbody>
...
function CierreTab({ vans, services, isAdmin }) {
  const [date, setDate] = useState(todayISO());
  ...
  const breakdown = useMemo(() => {
    return vans.map(van => {
      ...
      const byMethod = PAYMENT_METHODS.reduce((acc, m) => {
        ...
        acc[m] = {
          count: list.length,
          amount: list.reduce((sum, i) => sum + i.amount, 0),
          tips: list.reduce((sum, i) => sum + (i.tip || 0), 0),
        };
        return acc;
      }, {});
      const total = items.reduce((sum, i) => sum + i.amount, 0);
      const tips = items.reduce((sum, i) => sum + (i.tip || 0), 0);
      return { van, items, byMethod, total, tips, count: items.length };
    });
  }, [vans, services, date]);
  ...
  const grandByMethod = PAYMENT_METHODS.reduce((acc, m) => {
    acc[m] = breakdown.reduce((sum, b) => sum + b.byMethod[m].amount, 0);
    return acc;
  }, {});
  ...
  return (
    <div style={{ animation: 'fadeIn 0.3s ease' }}>
      ...
      <div style={styles.kpiGrid}>
        <KpiCard label="Servicios del día" value={grandCount} highlight />
        <KpiCard label="Total ventas" value={fmt(grandTotal)} highlight />
        {grandTips > 0 && <KpiCard label="Total propinas" value={fmt(grandTips)} />}
      </div>

      {grandTotal > 0 && (
        <div style={{ ...styles.card, marginTop: 16 }}>
          <h3 style={styles.cardH3}>Totales por método de pago</h3>
      ...
      {isAdmin && (
        <div style={{ marginTop: 28 }}>
          ...
          <div style={styles.vanGrid}>
            {breakdown.map(b => (
              <VanCierreCard key={b.van.id} data={b} />
            ))}
          </div>
        </div>
      )}
...
function VanCierreCard({ data }) {
  const { van, byMethod, total, tips, count } = data;
  return (
    <div style={styles.vanCard}>
      <div style={styles.vanCardHead}>
        <div>
          <div style={styles.vanCardName}>{van.name}</div>
          {van.groomer && <div style={styles.vanCardGroomer}>{van.groomer}</div>}
        </div>
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontSize: 11, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.08em' }}>{count} serv.</div>
          <div style={{ fontFamily: 'Fraunces, serif', fontSize: 24, fontWeight: 600, color: '#0f172a' }}>{fmt(total)}</div>
        </div>
      </div>
      {total === 0 ? (
        ...
        <>
            ...
            {PAYMENT_METHODS.map(m => {
              ...
              return (
                <div key={m} style={styles.vanMethodRow}>
                  ...
                  <span style={{ fontWeight: 600, color: '#0f172a' }}>{fmt(v.amount)}</span>
                </div>
              );
            })}
          </div>
          {tips > 0 && (
            <div style={styles.tipsRow}>
              <span style={{ fontSize: 12, color: '#64748b' }}>Propinas</span>
              <span style={{ fontWeight: 600, color: '#0f766e' }}>{fmt(tips)}</span>
            </div>
          )}
        </>
      )}
...
function SemanaTab({ vans, services, settings }) {
  ...
  const report = useMemo(() => {
    ...
    return vans.map(van => {
      ...
      const byMethod = PAYMENT_METHODS.reduce((acc, m) => {
        acc[m] = items.filter(i => i.method === m).reduce((sum, i) => sum + i.amount, 0);
        return acc;
      }, {});

      return { van, count: items.length, sales, tips, commission, tipShare, totalPay, byMethod };
    });
  }, [vans, services, start, end, settings]);

  const totals = report.reduce((acc, r) => ({
    sales: acc.sales + r.sales,
    tips: acc.tips + r.tips,
    commission: acc.commission + r.commission,
    tipShare: acc.tipShare + r.tipShare,
    totalPay: acc.totalPay + r.totalPay,
    count: acc.count + r.count,
  }), { sales: 0, tips: 0, commission: 0, tipShare: 0, totalPay: 0, count: 0 });

  const exportCSV = () => {
    const rows = [
      ['Reporte Semanal Grooming Vans'],
      [`Semana: ${start} a ${end}`],
      [`Comisión: ${settings.commissionPct}% · Propinas al groomer: ${settings.tipsToGroomer}%`],
      [],
      ['Van', 'Groomer', 'Servicios', 'Ventas', 'Efectivo', 'Zelle', 'Transferencia', 'Cheque', 'Propinas', 'Comisión', 'Parte propinas', 'A PAGAR'],
    ];
    report.forEach(r => {
      rows.push([
        r.van.name,
        ...
        r.count,
        r.sales.toFixed(2),
        r.byMethod['Efectivo'].toFixed(2),
        r.byMethod['Zelle'].toFixed(2),
        r.byMethod['Transferencia'].toFixed(2),
        r.byMethod['Cheque'].toFixed(2),
        r.tips.toFixed(2),
    ...
    rows.push([]);
    rows.push(['TOTAL', '', totals.count, totals.sales.toFixed(2), '', '', '', '', totals.tips.toFixed(2), totals.commission.toFixed(2), totals.tipShare.toFixed(2), totals.totalPay.toFixed(2)]);
    ...
    a.href = url;
    a.download = `reporte-semanal-${start}.csv`;
    a.click();
  ...
  return (
    <div style={{ animation: 'fadeIn 0.3s ease' }}>
      ...
      <div style={styles.card}>
        <div style={{ display: 'flex', gap: 16, alignItems: 'flex-end', flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: 200 }}>
            ...
            <p style={{ fontSize: 12, color: '#94a3b8', margin: '6px 0 0' }}>Semana mostrada: lunes a domingo</p>
          </div>
      ...
      <div style={styles.kpiGrid}>
        ...
        <KpiCard label="Propinas" value={fmt(totals.tips)} />
        <KpiCard label="Total a pagar groomers" value={fmt(totals.totalPay)} highlight accent />
      </div>
            ...
            <table style={styles.table}>
              <thead>
                <tr>
                  ...
                  <th style={{ ...styles.th, textAlign: 'right' }}>Servicios</th>
      ...
      <div style={{ marginTop: 24 }}>
        <SectionTitle eyebrow="Desglose por método" title="Cómo entró el dinero esta semana" />
        <div style={styles.card}>
          <div style={{ overflowX: 'auto' }}>
            <table style={styles.table}>
              <thead>
              ...
              <tbody>
                {report.map(r => (
                  <tr key={r.van.id} className="row-hover" style={styles.tr}>
                    <td style={styles.td}><strong>{r.van.name}</strong></td>
                    {PAYMENT_METHODS.map(m => (
                      <td key={m} style={{ ...styles.td, textAlign: 'right', color: r.byMethod[m] > 0 ? '#0f172a' : '#cbd5e1' }}>
                        {fmt(r.byMethod[m])}
                      </td>
                    ))}
                    <td style={{ ...styles.td, textAlign: 'right', fontWeight: 700 }}>{fmt(r.sales)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
...
function ConfigTab({ vans, setVans, settings, setSettings, services, setServices }) {
  ...
  return (
    <div style={{ animation: 'fadeIn 0.3s ease' }}>
      ...
      <div style={styles.card}>
        <h3 style={styles.cardH3}>Comisiones y propinas</h3>
        <div style={styles.formGrid}>
          <div>
            ...
            <p style={{ fontSize: 12, color: '#94a3b8', margin: '6px 0 0' }}>Porcentaje del total de ventas que se paga al groomer</p>
          </div>
          <div>
            ...
            <p style={{ fontSize: 12, color: '#94a3b8', margin: '6px 0 0' }}>Normalmente 100% — el groomer se queda con todas las propinas</p>
          </div>
        </div>
      </div>
      ...
      <div style={{ ...styles.card, marginTop: 16 }}>
        ...
        <p style={{ fontSize: 13, color: '#64748b', marginTop: 0 }}>
          Tu PIN para entrar al panel y ver toda la información. <strong>No lo compartas con los groomers.</strong>
        </p>
      ...
      <div style={{ ...styles.card, marginTop: 16 }}>
        ...
        <p style={{ fontSize: 13, color: '#64748b', marginTop: 0 }}>
          Cada groomer entra con su PIN y solo ve <strong>su propio registro y cierre</strong>. Cambia los PINs aquí.
        </p>
              ...
              <div key={v.id} style={styles.vanRow}>
                {editing ? (
                  <>
                    <input
                      value={editing.name}
                      onChange={e => setEditVan({ ...editVan, [v.id]: { ...editing, name: e.target.value } })}
                      placeholder="Van"
                      style={{ ...styles.input, flex: '0 0 100px' }}
                    />
                    <input
                      value={editing.groomer}
                      onChange={e => setEditVan({ ...editVan, [v.id]: { ...editing, groomer: e.target.value } })}
                    ...
                    <input
                      ...
                      placeholder="PIN"
                      style={{ ...styles.input, flex: '0 0 100px', fontFamily: 'monospace', letterSpacing: '0.2em', textAlign: 'center' }}
                    />
                  ...
                  <>
                    ...
                    <div style={{ flex: 1, minWidth: 140, color: v.groomer ? '#0f172a' : '#94a3b8' }}>{v.groomer || 'Sin groomer'}</div>
                    <div style={{ flex: '0 0 100px', fontFamily: 'monospace', color: '#475569', textAlign: 'center', background: '#fff', padding: '6px 10px', borderRadius: 6, border: '1px solid #e2e8f0', letterSpacing: '0.15em' }}>
                      {v.pin || '----'}
                    </div>
      ...
      <div style={{ ...styles.card, marginTop: 16, borderColor: '#fecaca' }}>
        ...
        <p style={{ fontSize: 13, color: '#64748b', marginTop: 0 }}>
          Hay {services.length} servicio{services.length === 1 ? '' : 's'} registrado{services.length === 1 ? '' : 's'} en total.
        </p>
...
function KpiCard({ label, value, highlight, accent }) {
  return (
    <div style={{
      ...
      ...(accent ? { background: '#0f766e', color: '#fff', borderColor: '#0f766e' } : {}),
    }}>
...
const styles = {
  ...
  app: {
    minHeight: '100vh',
    background: '#fafaf7',
    backgroundImage: 'radial-gradient(circle at 90% 0%, rgba(15,118,110,0.06) 0%, transparent 50%), radial-gradient(circle at 0% 100%, rgba(245,158,11,0.04) 0%, transparent 50%)',
  ...
  tabBtnActive: {
    color: '#0f766e', borderBottomColor: '#0f766e', fontWeight: 600,
  },
  ...
  eyebrow: {
    ...
    color: '#0f766e', fontWeight: 700, marginBottom: 4,
  },
  h2: {
    fontFamily: 'Fraunces, serif',
  ...
  cardH3: {
    ...
    margin: '0 0 14px', color: '#0f172a', letterSpacing: '-0.01em',
  },
  ...
  iconBtn: {
    ...
    padding: 6, marginLeft: 4, color: '#64748b',
    borderRadius: 6,
  },
  iconBtnGreen: {
    background: '#0f766e', border: 'none', cursor: 'pointer',
    padding: 6, marginLeft: 4, color: '#fff', borderRadius: 6,
  },
  ...
  methodAmount: {
    ...
    marginTop: 8, color: '#0f172a', letterSpacing: '-0.01em',
  },
  vanGrid: {
  ...
  vanRow: {
    ...
    padding: 10, background: '#fafaf7', borderRadius: 10,
  },
  suggestionsBox: {
    position: 'absolute',
    top: 'calc(100% + 4px)',
  ...
  loginScreen: {
    ...
    padding: '20px',
    background: '#fafaf7',
    backgroundImage: 'radial-gradient(circle at 90% 10%, rgba(15,118,110,0.10) 0%, transparent 50%), radial-gradient(circle at 10% 90%, rgba(245,158,11,0.06) 0%, transparent 50%)',
    fontFamily: 'Manrope, -apple-system, sans-serif',
  },
  loginCard: {
    width: '100%',
