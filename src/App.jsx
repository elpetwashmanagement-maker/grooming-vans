...
function AppointmentsTab({ appointments, vans, clients, pets, session, settings, isAdmin, canViewAllSchedule, updateApptStatus, addAppointment, addClient, addPet, refreshAppointments, deleteAppt, servicePrices, deposits = [], setDeposits = () => {}, groomers = [] }) {
  ...
  const [editingCompletedAppt, setEditingCompletedAppt] = useState(null);
  const [editPayForm, setEditPayForm] = useState({ tip: 0, method: 'Cash', price: 0 });
                ...
                {editingPetInline && (
                  <div style={{ marginTop: 10, padding: 12, background: '#fffbeb', borderRadius: 10, border: '1.5px solid #f59e0b' }}>
                    ...
                    <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
                      ...
                      <button onClick={async () => {
                        const petToUpdate = pets.find(p => String(p.id) === String(editingPetInline));
                        if (petToUpdate) await updatePet({ ...petToUpdate, size: editPetInlineForm.size, hair_type: editPetInlineForm.hair_type, breed: editPetInlineForm.breed, weight: editPetInlineForm.weight });
